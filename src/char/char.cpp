...
int char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p){
	...
	// reputations
	StringBuf_Clear(&buf);
	StringBuf_Printf(&buf, "REPLACE INTO `%s` (`char_id`, `reputation_id`, `amount`) VALUES ", schema_config.reputations_table);
	diff = 0;
	for (i = 0; i < MAX_REPUTATIONS; ++i) {
		if (memcmp(&p->reputations[i], &cp->reputations[i], sizeof(s_reputation)))
		{
			if (diff) {
				StringBuf_AppendStr(&buf, ","); // not the first hotkey
			}
			StringBuf_Printf(&buf, "('%d', '%u', '%d')", char_id, p->reputations[i].id, p->reputations[i].amount);
			diff = 1;
		}
	}
	if (diff) {
		if (SQL_ERROR == Sql_QueryStr(sql_handle, StringBuf_Value(&buf)))\
		{
			...
			strcat(save_status, " reputations");
		}
	}
...
int char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	// read reputations
	// `reputations` (`char_id`, `reputation_id`, `amount`)
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `reputation_id`, `amount` from `%s` WHERE `char_id` = ? ORDER BY `reputation_id` ASC", schema_config.reputations_table)
		|| SQL_ERROR == SqlStmt_BindParam(stmt, 0, SQLDT_INT, &char_id, 0)
		|| SQL_ERROR == SqlStmt_Execute(stmt)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_UINT32, &tmp_reputation.id, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 1, SQLDT_INT32, &tmp_reputation.amount, 0, NULL, NULL))
	{
		SqlStmt_ShowDebug(stmt);
	}

	int reputation_num = 0;
	while (SQL_SUCCESS == SqlStmt_NextRow(stmt))
	{
		if (reputation_num < MAX_REPUTATIONS) {
			memcpy(&p->reputations[reputation_num], &tmp_reputation, sizeof(tmp_reputation));
			reputation_num++;
		}
		else
			ShowWarning("mmo_char_fromsql: ignoring invalid reputation (id=%u, amount=%d) of character %s (AID=%d, CID=%d)\n", tmp_reputation.id, tmp_reputation.amount, p->name, p->account_id, p->char_id);
	}
	StringBuf_AppendStr(&msg_buf, " reputations");
...
enum e_char_del_response char_delete(struct char_session_data* sd, uint32 char_id){
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id` = '%d'", schema_config.reputations_table, char_id))
		Sql_ShowDebug(sql_handle);
...
bool char_checkdb(void){
	...
	const char* sqltable[] = {
		...
		schema_config.reputations_table,
	};
	...
	// checking reputations_table
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`, `reputation_id`, `amount` FROM `%s` LIMIT 1;", schema_config.reputations_table)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	Sql_FreeResult(sql_handle);
...
void char_sql_config_read(const char* cfgName) {
	...
	while(fgets(line, sizeof(line), fp)) {
		...
		else if(!strcmpi(w1, "reputations_table"))
			safestrncpy(schema_config.reputations_table, w2, sizeof(schema_config.reputations_table));
...
//	schema_config.db_use_sqldbs;
	...
	safestrncpy(schema_config.reputations_table, "reputations", sizeof(schema_config.reputations_table));
}
