...
ACMD_FUNC(mobinfo)
{
	...
	for (uint16 k = 0; k < count; k++) {
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			itemldata.item.nameid = id->nameid;
...
ACMD_FUNC(reformui)
{
	...
	clif_displaymessage(fd, "This command requires packet version 2020-09-16 or newer.");
	return -1;
#else
	t_itemid initiator;

	if (sscanf(message, "%11u", &initiator) == 1) {
		clif_open_reform_ui(sd, initiator);
		return 0;
	}
	else {
		clif_displaymessage(fd, "Reform ui command requires initiator id.");
		return -1;
	}
#endif
}
...
ACMD_FUNC(enchantui)
{
	...
	clif_displaymessage(fd, "This command requires packet version 2020-11-18 or newer.");
	return -1;
#else
	uint32 tableNumber;

	if (sscanf(message, "%11u", &tableNumber) == 1) {
		clif_open_enchant_ui(sd, tableNumber);
		return 0;
	}
	else {
		clif_displaymessage(fd, "Enchant ui command requires table number.");
		return -1;
	}
#endif
}
...
ACMD_FUNC(reputation)
{
	...
	if (sscanf(message, "%11u %11d", &id, &amount) == 2) {
		pc_reputation_modify(sd, id, amount);
		clif_displaymessage(fd, "Adjusted reputation.");
		return 0;
	}
	else {
		return -1;
...
ACMD_FUNC(equipoff) {
	...
	if (strcmp(parent_cmd, "equipoff") == 0) {
		...
		clif_displaymessage(fd,  msg_txt(sd, 1522)); // Use @equipon to disable the system.
		clif_displaymessage(fd, msg_txt(sd, 1520)); // EquipOff activated!

	} else if (strcmp(parent_cmd, "equipon") == 0) {
		sd->state.equiphide = 0;
		clif_refresh(sd);
		clif_status_change(&sd->bl, EFST_EQUIP_HIDE, 0, INVALID_TIMER, 0, 0, 0);
		clif_displaymessage(fd, msg_txt(sd, 1521)); // EquipOff disabled!
		clif_refreshlook(&sd->bl, sd->bl.id, LOOK_HEAD_TOP, sd->status.head_top, SELF);
		clif_refreshlook(&sd->bl, sd->bl.id, LOOK_HEAD_MID, sd->status.head_mid, SELF);
		clif_refreshlook(&sd->bl, sd->bl.id, LOOK_HEAD_BOTTOM, sd->status.head_bottom, SELF);
		clif_refreshlook(&sd->bl, sd->bl.id, LOOK_ROBE, sd->status.robe, SELF);
