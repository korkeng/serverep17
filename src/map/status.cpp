...
t_tick status_get_sc_def(struct block_list *src, struct block_list *bl, enum sc_type type, int rate, t_tick tick, unsigned char flag)
{
	...
	switch (type) {
		case SC_POISON:
		case SC_DPOISON:
		        if (bl->type == BL_PC && status->luk >= 3000)
                    return 0;
		        if (bl->type == BL_PC && status->vit >= 3000)
                    return 0;
			tick_def2 = status_src->luk*0; // Caster can increase final duration with luk
			break;
		case SC_STUN:
		        if (bl->type == BL_PC && status->luk >= 3000)
                    return 0;
		        if (bl->type == BL_PC && status->vit >= 3000)
                    return 0;
			tick_def2 = status_src->luk*-4; // Caster can increase final duration with luk
			break;
		case SC_SILENCE:
		        if (bl->type == BL_PC && status->luk >= 3000)
                    return 0;
		        if (bl->type == BL_PC && status->vit >= 3000)
                    return 0;
		        if (bl->type == BL_PC && status->int_ >= 3000)
                    return 0;
			tick_def2 = status_src->luk*19; // Caster can increase final duration with luk
			break;
		case SC_BLEEDING:
		        if (bl->type == BL_PC && status->agi >= 3000)
                    return 0;
		        if (bl->type == BL_PC && status->luk >= 3000)
                    return 0;
		        if (bl->type == BL_PC && status->vit >= 3000)
                    return 0;
			tick_def2 = status_src->luk*20; // Caster can increase final duration with luk
			break;
		case SC_SLEEP:
		        if (bl->type == BL_PC && status->agi >= 3000)
                    return 0;
		        ...
		        if (bl->type == BL_PC && status->int_ >= 3000)
                    return 0;
			tick_def2 = status_src->luk*1; // Caster can increase final duration with luk
			break;
		case SC_STONEWAIT:
			if (bl->type == BL_PC && status->luk >= 3000)
                    return 0;
		...
		case SC_FREEZE:
		        if (bl->type == BL_PC && status->luk >= 3000)
                    return 0;
			tick_def2 = status_src->luk*7; // Caster can increase final duration with luk
			break;
		case SC_CURSE:
		        if (bl->type == BL_PC && status->luk >= 3000)
                    return 0;
		...
		case SC_CONFUSION:
		        ...
		        if (bl->type == BL_PC && status->int_ >= 3000)
                    return 0;
			tick_def2 = status_src->luk*-3; // Caster can increase final duration with luk
			break;
		case SC_DECREASEAGI:
			if (sd)
