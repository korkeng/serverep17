...
bool pc_reputation_modify(map_session_data* sd, uint32 id, int32 amount)
{
	nullpo_retr(false, sd);

	if (id > MAX_REPUTATIONS) {
		ShowWarning("pc_reputation_modify: Tried to modify reputation with Id '%u' which is over MAX_REPUTATIONS for player '%s' (AID=%d, CID=%d)\n", id, sd->status.name, sd->status.account_id, sd->status.char_id);
		return false;
	}

	const auto& reputationGroup = reputation_db.reputationGroupsTable[id];
	if (reputationGroup == nullptr) {
		ShowWarning("pc_reputation_modify: Tried to modify nonexistent reputation Id '%u' for player '%s' (AID=%d, CID=%d)\n", id, sd->status.name, sd->status.account_id, sd->status.char_id);
		return false;
	}

	sd->status.reputations[id - 1].id = id;
	sd->status.reputations[id - 1].amount += amount;
	cap_value(sd->status.reputations[id - 1].amount, reputationGroup->min, reputationGroup->max);
	clif_reputation_update(sd, &sd->status.reputations[id - 1], 1);

	return true;
}

int32 pc_reputation_get(map_session_data* sd, uint32 id)
{
	nullpo_retr(-1, sd);

	if (id > MAX_REPUTATIONS) {
		ShowWarning("pc_reputation_modify: Tried to get reputation with Id '%u' which is over MAX_REPUTATIONS for player '%s' (AID=%d, CID=%d)\n", id, sd->status.name, sd->status.account_id, sd->status.char_id);
		return 0;
	}

	const auto& reputationGroup = reputation_db.reputationGroupsTable[id];
	if (reputationGroup == nullptr) {
		ShowWarning("pc_reputation_modify: Tried to get nonexistent reputation Id '%u' for player '%s' (AID=%d, CID=%d)\n", id, sd->status.name, sd->status.account_id, sd->status.char_id);
		return 0;
	}

	return sd->status.reputations[id - 1].amount;
}

void pc_reputation_init(map_session_data* sd) {
	nullpo_retv(sd);

	// Check if reputation isn't initialized for this character.
	if (sd->status.reputations[0].id == 0) {
		int i = 0;
		for (const auto& reputation : reputation_db.reputationGroupsTable) {
			sd->status.reputations[i].id = reputation.first;
			sd->status.reputations[i].amount = 0;
		}
	}

	clif_reputation_update(sd, sd->status.reputations, MAX_REPUTATIONS);
}
...
uint64 ReputationDatabase::parseBodyNode(const ryml::NodeRef& node) {
	std::shared_ptr<s_reputation_group> reputationGroup;
	uint32 id;

	if (this->nodeExists(node, "Id")) {
		this->asUInt32(node, "Id", id);
		if (id > MAX_REPUTATIONS) {
			this->invalidWarning(node["Id"], "Reputation ID cannot be greater than to MAX_REPUTATIONS (%d).\n", MAX_REPUTATIONS);
			return 0;
		}
	}
	else {
		this->invalidWarning(node, "Reputation group must have Id property.\n");
		return 0;
	}
	reputationGroup = this->reputationGroupsTable[id];
	if (reputationGroup == nullptr) {
		reputationGroup = std::make_shared<s_reputation_group>();
	}
	reputationGroup->id = id;
	if (this->nodeExists(node, "Min")) {
		this->asInt32(node, "Min", reputationGroup->min);
	}
	if (this->nodeExists(node, "Max")) {
		this->asInt32(node, "Max", reputationGroup->max);
	}
	if (this->nodeExists(node, "Name")) {
		this->asString(node, "Name", reputationGroup->name);
	}
	this->reputationGroupsTable[id] = reputationGroup;

	return 1;
}

void ReputationDatabase::loadingFinished() {
