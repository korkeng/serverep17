...
BUILDIN_FUNC(open_reform_ui)
{
	...
	if (script_rid2sd(sd)) {
		...
		if (sd->state.reform_itemid != 0) {
			ShowWarning("buildin_open_reform_ui: Tried to open reform UI for char ID %u while player already has reform UI open.\n", sd->status.char_id);
			return SCRIPT_CMD_SUCCESS;
		}

		if (script_hasdata(st, 2)) {
			initiator = script_getnum(st, 2);
		}
		else if (current_equip_item_index != -1) {
			initiator = sd->inventory.u.items_inventory[current_equip_item_index].nameid;
		}
		else {
			ShowError("buildin_open_reform_ui: Command was called without proper context, initiator item ID is required when not called from item script.\n");
			return SCRIPT_CMD_FAILURE;
		}

		sd->state.reform_itemid = initiator;
		clif_open_reform_ui(sd, initiator);
...
BUILDIN_FUNC(open_enchant_ui)
{
	map_session_data* sd;

	if (script_rid2sd(sd)) {
		uint32 tableNumber = script_getnum(st, 2);

		if (sd->state.enchant_itemid == true) {
			ShowWarning("buildin_open_enchant_ui: Tried to open enchant UI for char ID %u while player already has enchant UI open.\n", sd->status.char_id);
			return SCRIPT_CMD_SUCCESS;
		}

		sd->state.enchant_itemid = tableNumber;
...
BUILDIN_FUNC(reputation_modify)
{
	map_session_data* sd;

	if (script_rid2sd(sd)) {
		uint32 id = script_getnum(st, 2);
		int32 amount = script_getnum(st, 3);

		if (!pc_reputation_modify(sd, id, amount)) {
			ShowError("buildin_reputation_modify: Failed to manipulate reputation.\n");
			return SCRIPT_CMD_FAILURE;
		}
...
BUILDIN_FUNC(reputation_get)
{
	map_session_data* sd;

	if (script_rid2sd(sd)) {
		uint32 id = script_getnum(st, 2);
		int32 amount = pc_reputation_get(sd, id);
...
// <--- [zBuffer] List of unit control commands
	...
	BUILDIN_DEF(open_reform_ui, "?"),
	BUILDIN_DEF(open_enchant_ui, "i"),
	BUILDIN_DEF(reputation_modify, "ii"),
	BUILDIN_DEF(reputation_get, "i"),
