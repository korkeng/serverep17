...
DEFINE_PACKET_HEADER(ZC_OPEN_REFORM_UI, 0xb8f);

struct PACKET_CZ_CLOSE_REFORM_UI {
	int16 PacketType;
};
DEFINE_PACKET_HEADER(CZ_CLOSE_REFORM_UI, 0xb90);

struct PACKET_CZ_REFORM_UI_REFORM {
	int16 PacketType;
	uint32 initiatorId;
	uint16 index;
};
DEFINE_PACKET_HEADER(CZ_REFORM_UI_REFORM, 0xb91);

struct PACKET_ZC_REFORM_ACK {
	int16 PacketType;
	uint16 PacketLength;
	uint8 result;
};
DEFINE_PACKET_HEADER(ZC_REFORM_ACK, 0xb92);

struct PACKET_ZC_OPEN_ENCHANT_UI {
	int16 PacketType;
	char ui;
	uint32 tableNumber;
	uint32 unknown;
};
DEFINE_PACKET_HEADER(ZC_OPEN_ENCHANT_UI, 0xb9a);

struct PACKET_CZ_CLOSE_ENCHANT_UI {
	int16 PacketType;
};
DEFINE_PACKET_HEADER(CZ_CLOSE_ENCHANT_UI, 0xba0);

struct PACKET_CZ_ENCHANT_SIMPLE {
	int16 PacketType;
	uint32 tableNumber;
	uint32 unknown;
	uint16 index;
};
DEFINE_PACKET_HEADER(CZ_ENCHANT_SIMPLE, 0xb9b);

struct PACKET_CZ_ENCHANT_PERFECT {
	int16 PacketType;
	uint32 tableNumber;
	uint32 unknown;
	uint16 index;
	uint32 selectedEnchantId;
};
DEFINE_PACKET_HEADER(CZ_ENCHANT_PERFECT, 0xb9c);

struct PACKET_CZ_ENCHANT_UPGRADE {
	int16 PacketType;
	uint32 tableNumber;
	uint32 unknown;
	uint16 index;
...
DEFINE_PACKET_HEADER(CZ_ENCHANT_UPGRADE, 0xb9d);

struct PACKET_CZ_ENCHANT_RESET {
	int16 PacketType;
	uint32 tableNumber;
	uint32 unknown;
	uint16 index;
};
DEFINE_PACKET_HEADER(CZ_ENCHANT_RESET, 0xb9e);

struct PACKET_ZC_ENCHANT_ACK {
	int16 PacketType;
	uint32 messageId;
	uint32 resultEnchantId;
};
DEFINE_PACKET_HEADER(ZC_ENCHANT_ACK, 0xb9f);

struct PACKET_ZC_REPUTE_LIST_REPUTATION_BLOCK {
	uint32 reputationId;
	int32 unknown;
	int64 amount;
};

struct PACKET_ZC_REPUTE_LIST {
	int16 PacketType;
	uint16 PacketLength;
	uint8 unknown;
	PACKET_ZC_REPUTE_LIST_REPUTATION_BLOCK reputations[];
};
DEFINE_PACKET_HEADER(ZC_REPUTE_LIST, 0xb8d);
