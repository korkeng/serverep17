...
void clif_parse_WantToConnection(int fd, struct map_session_data* sd)
{
	...
	client_tick = RFIFOQ(fd, packet_db[cmd].pos[3]);
#else
...
void clif_parse_ViewPlayerEquip(int fd, struct map_session_data* sd)
{
	...
	if( (pc_readaccountreg(tsd, add_str("#PRIVATE_VIEWEQUIP")) > 0 && tsd->status.show_equip) || (pc_has_permission(sd, PC_PERM_VIEW_EQUIPMENT) && (pc_readaccountreg(tsd,add_str("#PRIVATE_VIEWEQUIP")) < 1 || pc_get_group_level(sd) >= 80)) )
...
void clif_parse_SkillSelectMenu(int fd, struct map_session_data *sd) {
	...
	if (sd->menuskill_id == SA_AUTOSPELL) {
		...
		if (sd->state.check_equip_skill) {
			pcb_process_selection(sd, RFIFOW(fd, info->pos[1]));
			return;
		}
		if (pc_istrading(sd)) {
			clif_skill_fail(sd, sd->ud.skill_id, USESKILL_FAIL_LEVEL, 0);
...
**/
int clif_vend2(struct map_session_data *sd, int skill_lv) {
...
void clif_open_reform_ui(map_session_data* sd, t_itemid initiatorId)
{
...
void clif_parse_close_reform_ui(int fd, map_session_data* sd)
{
#if PACKETVER >= 20200916
	nullpo_retv(sd);

	sd->state.reform_itemid = 0;
#endif
}

void clif_parse_reform_ui_reform(int fd, map_session_data* sd)
{
#if PACKETVER >= 20200916
	nullpo_retv(sd);

	PACKET_CZ_REFORM_UI_REFORM* p = (PACKET_CZ_REFORM_UI_REFORM*)RFIFOP(fd, 0);
	t_itemid initiatorId = p->initiatorId;
	uint16 index = server_index(p->index);

	// Check if the reform UI is open
	if (sd->state.reform_itemid != p->initiatorId) {
		return;
	}

	reform_do_reform(sd, initiatorId, index);
#endif
}

void clif_preview_item(map_session_data* sd, uint16 index)
{
	nullpo_retv(sd);
	...
	PACKET_ZC_ITEM_PREVIEW p{};
	p.packetType = HEADER_ZC_ITEM_PREVIEW;
	p.index = (int16)client_index(index);
	p.refiningLevel = (int16)item->refine;
	memcpy(&p.slot.card, item->card, sizeof(t_itemid) * MAX_SLOTS);
	memcpy(&p.option_data, item->option, sizeof(s_item_randomoption) * MAX_ITEM_RDM_OPT);

	clif_send(&p, sizeof(p), &sd->bl, SELF);
}

void clif_reform_ack(map_session_data* sd, bool success) {
#if PACKETVER >= 20200916
	nullpo_retv(sd);

	PACKET_ZC_REFORM_ACK p{};
	p.PacketType = HEADER_ZC_REFORM_ACK;
	p.PacketLength = sizeof(PACKET_ZC_REFORM_ACK);
	p.result = success ? 0 : 1;

	clif_send(&p, sizeof(PACKET_ZC_REFORM_ACK), &sd->bl, SELF);

	sd->state.reform_itemid = 0;
#endif
}

void clif_open_enchant_ui(map_session_data* sd, uint32 tableNumber) {
#if PACKETVER >= 20201118
	nullpo_retv(sd);

	PACKET_ZC_OPEN_ENCHANT_UI p {};

	p.PacketType = HEADER_ZC_OPEN_ENCHANT_UI;
	p.ui = 0xa; // UI won't open with other values
...
void clif_parse_close_enchant_ui(int fd, map_session_data* sd) {
#if PACKETVER >= 20201118
	nullpo_retv(sd);

	sd->state.enchant_itemid = 0;
#endif
}

void clif_enchant_ack(map_session_data* sd, bool success, t_itemid nameid) {
#if PACKETVER >= 20201118
	nullpo_retv(sd);

	PACKET_ZC_ENCHANT_ACK p{};

	p.PacketType = 0xb9f;
	p.messageId = success ? 3857 : 3858;
	p.resultEnchantId = (uint32)nameid;
...
void clif_parse_enchant_simple(int fd, map_session_data* sd) {
#if PACKETVER >= 20201118
	nullpo_retv(sd);

	PACKET_CZ_ENCHANT_SIMPLE* p = (PACKET_CZ_ENCHANT_SIMPLE*)RFIFOP(fd, 0);

	if (sd->state.enchant_itemid != p->tableNumber) {
		return;
	}

	enchant_apply_simple(sd, p->tableNumber, server_index(p->index));
#endif
}

void clif_parse_enchant_perfect(int fd, map_session_data* sd) {
#if PACKETVER >= 20201118
	nullpo_retv(sd);

	PACKET_CZ_ENCHANT_PERFECT* p = (PACKET_CZ_ENCHANT_PERFECT*)RFIFOP(fd, 0);

	if (sd->state.enchant_itemid != p->tableNumber) {
		return;
	}

	enchant_apply_perfect(sd, p->tableNumber, server_index(p->index), p->selectedEnchantId);
#endif
}

void clif_parse_enchant_upgrade(int fd, map_session_data* sd) {
#if PACKETVER >= 20201118
	nullpo_retv(sd);

	PACKET_CZ_ENCHANT_UPGRADE* p = (PACKET_CZ_ENCHANT_UPGRADE*)RFIFOP(fd, 0);

	if (sd->state.enchant_itemid != p->tableNumber) {
		return;
	}

	enchant_apply_upgrade(sd, p->tableNumber, server_index(p->index), p->slot);
#endif
}

void clif_parse_enchant_reset(int fd, map_session_data* sd) {
#if PACKETVER >= 20201118
	nullpo_retv(sd);

	PACKET_CZ_ENCHANT_RESET* p = (PACKET_CZ_ENCHANT_RESET*)RFIFOP(fd, 0);

	if (sd->state.enchant_itemid != p->tableNumber) {
		return;
	}

	enchant_reset(sd, p->tableNumber, server_index(p->index));
#endif
}

void clif_reputation_update(map_session_data* sd, s_reputation reputations[], int n) {
#if PACKETVER >= 20201202
	nullpo_retv(sd);

	int fd = sd->fd;
	int16 size = sizeof(PACKET_ZC_REPUTE_LIST) + (n * sizeof(PACKET_ZC_REPUTE_LIST_REPUTATION_BLOCK));

	WFIFOHEAD(fd, size);
	PACKET_ZC_REPUTE_LIST* p = (PACKET_ZC_REPUTE_LIST*)WFIFOP(fd, 0);
	p->PacketType = HEADER_ZC_REPUTE_LIST;
	p->PacketLength = size;
	p->unknown = 1; // Must be 1 or client won't react to this packet
	for (int i = 0; i < n; ++i) {
		p->reputations[i].reputationId = reputations[i].id;
		p->reputations[i].amount = reputations[i].amount;
		p->reputations[i].unknown = 0;
	}
