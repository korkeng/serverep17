...
struct map_session_data {
	...
	struct s_state {
		...
		t_itemid reform_itemid;
		t_itemid enchant_itemid;
		bool roulette_open;
	} state;
	struct {
...
static bool pc_cant_act2( struct map_session_data* sd ){
	return sd->state.vending || sd->state.buyingstore || (sd->sc.opt1 && sd->sc.opt1 != OPT1_BURNING)
		...
		|| sd->state.reform_itemid != 0 || sd->state.reform_itemid != 0
...
class ReputationDatabase : public YamlDatabase {
public:
	ReputationDatabase() : YamlDatabase("REPUTATION_DB", 1) {

	}

	std::map<uint32, std::shared_ptr<s_reputation_group>> reputationGroupsTable;

	void clear() {
		reputationGroupsTable.clear();
	}
